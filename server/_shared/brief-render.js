// Deterministic renderer for the AlsaGlobal Brief magazine.
//
...
function logoRef({ size, color }) {
  ...
  return (
    ...
    `aria-label="AlsaGlobal"${styleAttr}>` +
...
function digestRunningHead(dateShort, label) {
  return (
    ...
    logoRef({ size: 22 }) +
    ` · AlsaGlobal Brief · ${escapeHtml(dateShort)} ·` +
    '</span>' +
...
function renderStoryPage({ story, rank, palette, pageIndex, totalPages, issueDate }) {
  ...
  return (
    ...
    logoRef({ size: 28 }) +
    '<span class="mono">AlsaGlobal Brief</span>' +
    '</div>' +
    `<div class="page-number mono">${pad2(pageIndex)} / ${pad2(totalPages)}</div>` +
    '</section>'
  );
...
const STYLE_BLOCK = `<style>
  :root {
    /* AlsaGlobal brand palette — aligned with /pro landing + dashboard.
...
(function() {
  ...
  btn.addEventListener('click', async function() {
    ...
    try {
      var shareTitle = 'AlsaGlobal Brief';
      var shareText = 'My AlsaGlobal Brief for today:';
      if (navigator.share) {
        try {
...
function redactForPublic(data) {
  ...
  return {
    ...data,
    user: { ...data.user, name: 'AlsaGlobal' },
    digest: {
      ...data.digest,
      lead: safeLead,
      signals: safeSignals,
    ...
    stories: data.stories.map((s) => ({
      ...s,
      whyMatters: 'Subscribe to AlsaGlobal Brief to see the full editorial on this story.',
    })),
  };
}
...
export function renderBriefMagazine(envelope, options = {}) {
  ...
  const title = `AlsaGlobal Brief · ${escapeHtml(dateLong)}`;
  ...
  const publicStripHref = `/ ? `?ref=${encodeURIComponent(refCode)}` : ''}`;
  const publicStripHtml = publicMode
    ? '<div class="wm-public-strip">'
      + '<span>AlsaGlobal Brief \u00b7 shared issue</span>'
