...
export function getProviderCredentials(
  ...
  if (provider === 'gemini') {
    ...
    // Default: gemini-3.5-flash (fast + high quality). Override via GEMINI_MODEL env var.
    // Get a key at https://aistudio.google.com/apikey
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) return null;
    return {
      apiUrl: 'https://generativelanguage.googleapis.com/v1beta/openai/chat/completions',
      model: overrides.model || process.env.GEMINI_MODEL || 'gemini-3.5-flash',
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
      },
      // Disable internal thinking. Without this, Gemini 2.5 models burn the
      // entire max_tokens budget on internal reasoning and emit no output.
  ...
  if (provider === 'groq') {
    ...
    return {
      ...
      // Upgraded default: 70B is free on Groq and dramatically better than 8B.
      model: overrides.model || process.env.GROQ_MODEL || 'llama-3.3-70b-versatile',
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
      },
  ...
  if (provider === 'openrouter') {
    ...
    return {
      ...
      // Upgraded default: Gemini 2.5 Pro for best quality on reasoning tasks.
      model: overrides.model || process.env.OPENROUTER_MODEL || 'google/gemini-2.5-pro',
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
