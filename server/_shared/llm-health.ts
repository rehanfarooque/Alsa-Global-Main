 ...
 */
async function probe(_url: string): Promise<boolean> {
  // AlsaGlobal: pre-probing some hosts (Google, etc.) gives false negatives on
  // Windows/Node fetch. Trust the actual API call instead — it'll surface real
  // 401/429/5xx errors directly to the summarize handler.
  return true;
}
