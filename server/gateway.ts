...
import type { ServerOptions } from '../src/generated/server/alsaglobal/seismology/v1/service_server';
...
export function createDomainGateway(
  ...
  return async function handler(originalRequest: Request, ctx?: GatewayCtx): Promise<Response> {
    ...
    // verified Pro tool fetch never needs an `X-AlsaGlobal-Key`. If
    ...
    const wmKey =
      request.headers.get('X-AlsaGlobal-Key') ??
