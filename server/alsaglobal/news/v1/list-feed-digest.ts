...
} from '../../../../src/generated/server/alsaglobal/news/v1/service_server';
...
// Process-level cache for no-Redis environments (self-hosted / local dev).
// Serves stale data instantly while a background rebuild runs.
const PROC_CACHE_TTL_MS = 15 * 60 * 1000;
const _procCache = new Map<string, { data: ListFeedDigestResponse; ts: number }>();
...
export async function listFeedDigest(
  ...
  // Fast path: serve from process-level cache when Redis isn't available.
  // This lets self-hosted installs serve news instantly after the first build.
  const procHit = _procCache.get(fallbackKey);
  if (procHit && Date.now() - procHit.ts < PROC_CACHE_TTL_MS) {
    // Trigger a background refresh when cache is > 10 min old
    if (Date.now() - procHit.ts > 10 * 60 * 1000 && !_procInFlight.has(fallbackKey)) {
      const bg = buildDigest(variant, lang).then((r) => {
        const n = Object.values(r.categories).reduce((s, b) => s + b.items.length, 0);
        if (n > 0) _procCache.set(fallbackKey, { data: r, ts: Date.now() });
        return n > 0 ? r : null;
      }).finally(() => _procInFlight.delete(fallbackKey));
      _procInFlight.set(fallbackKey, bg);
    }
    return procHit.data;
  }

  // Coalesce concurrent cold-path builds so we don't fan out 50+ RSS fetches
  // for each simultaneous browser request on first load.
  const existing = _procInFlight.get(fallbackKey);
  if (existing) {
    const result = await existing;
    return result ?? fallbackDigestCache.get(fallbackKey)?.data ?? empty();
  }

  try {
    const buildPromise = cachedFetchJson<ListFeedDigestResponse>(
      digestCacheKey,
      900,
      async () => {
      ...
      if (fresh) {
        _procCache.set(fallbackKey, { data: fresh, ts: Date.now() });
    ...
    _procInFlight.set(fallbackKey, buildPromise);
    ...
    _procInFlight.delete(fallbackKey);
    markNoCacheResponse(ctx.request);
    return fallbackDigestCache.get(fallbackKey)?.data ?? empty();
  }
}
