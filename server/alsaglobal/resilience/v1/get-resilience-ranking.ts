...
} from '../../../../src/generated/server/alsaglobal/resilience/v1/service_server';
...
export const getResilienceRanking: ResilienceServiceHandler['getResilienceRanking'] = async (
  ...
  // budget. Gated on a valid seed API key in X-AlsaGlobal-Key (the same
  ...
  const forceRefresh = (() => {
    ...
    const wmKey = ctx.request.headers.get('X-AlsaGlobal-Key') ?? '';
    ...
    if (!allowed.has(wmKey)) {
      console.warn('[resilience] refresh=1 rejected: X-AlsaGlobal-Key not in seed allowlist');
      return false;
    }
    return true;
  })();
