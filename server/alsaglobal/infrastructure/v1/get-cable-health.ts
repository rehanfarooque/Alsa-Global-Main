...
async function fetchNgaWarnings(): Promise<NgaWarning[] | null> {
  try {
    ...
    return Array.isArray(data) ? data : ((data as { 'broadcast-warn'?: NgaWarning[]; broadcast_warn?: NgaWarning[] })?.['broadcast-warn'] ?? (data as { broadcast_warn?: NgaWarning[] })?.broadcast_warn ?? []);
  } catch {
...
export function processNgaSignals(warnings: NgaWarning[]): Signal[] {
  ...
  for (const warning of cableWarnings) {
    ...
    // NGA active warnings may have been issued months/years ago for ongoing operations.
    // Use the signal timestamp as now if the issued date is more than 7 days old, so
    // the recencyWeight calculation doesn't discard all long-running cable advisories.
    const signalTs = (ts && (Date.now() - ts) < 7 * 86400000) ? ts : Date.now() - 12 * 3600000;
    ...
    if (isFault) {
      signals.push({
        ...
        ttlSeconds: 30 * 86400,
        kind: 'operator_fault',
        evidence: [{ source: 'NGA', summary: `Fault/damage reported: ${summaryText}`, ts: signalTs }],
      });
    } else {
      signals.push({
        cableId,
        ...
        ttlSeconds: 30 * 86400,
        kind: 'cable_advisory',
        evidence: [{ source: 'NGA', summary: `Cable advisory: ${summaryText}`, ts: signalTs }],
      });
    }
