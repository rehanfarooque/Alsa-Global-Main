...
import type {
  ...
  NavigationalWarning,
  AisDisruption,
} from '../../../../src/generated/server/alsaglobal/maritime/v1/service_server';
...
async function fetchChokepointData(): Promise<ChokepointFetchResult> {
  ...
  // Only mark unavailable if BOTH live data sources fail (nav + vessel).
  // Missing transit summaries (Redis empty) is acceptable — chokepoints are still
  // useful with their hardcoded threat config even without daily transit counts.
  const upstreamUnavailable = (navFailed && vesselFailed)
    || (navFailed && disruptions.length === 0 && warnings.length === 0);
