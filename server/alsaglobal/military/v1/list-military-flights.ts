import type {
  ...
  MilitaryOperator,
  MilitaryConfidence,
} from '../../../../src/generated/server/alsaglobal/military/v1/service_server';
...
// In-memory OAuth2 token cache for OpenSky — avoids a token fetch per request.
let _openskyToken: { token: string; expiresAt: number } | null = null;

async function getOpenSkyToken(): Promise<string | null> {
  const clientId = process.env.OPENSKY_CLIENT_ID;
  const clientSecret = process.env.OPENSKY_CLIENT_SECRET;
  if (!clientId || !clientSecret) return null;
  if (_openskyToken && Date.now() < _openskyToken.expiresAt) return _openskyToken.token;
  try {
    ...
    _openskyToken = { token: data.access_token, expiresAt: Date.now() + ((data.expires_in ?? 300) - 30) * 1000 };
    return _openskyToken.token;
  } catch {
    return null;
  }
}
...
): Promise<{ states?: Array<[string, string, ...unknown[]]> } | null> {
  const token = await getOpenSkyToken();
  const params = new URLSearchParams({ lamin: String(lamin), lamax: String(lamax), lomin: String(lomin), lomax: String(lomax) });
  const url = `${OPENSKY_API_BASE}/states/all?${params}`;
  const headers: Record<string, string> = { Accept: 'application/json' };
  if (token) headers.Authorization = `Bearer ${token}`;
  try {
    const resp = await fetch(url, { headers, signal: AbortSignal.timeout(UPSTREAM_TIMEOUT_MS) });
    if (!resp.ok) return null;
    return await resp.json() as { states?: Array<[string, string, ...unknown[]]> };
  } catch {
    return null;
  }
}
...
export async function listMilitaryFlights(
  ...
  try {
    // Default to global bbox when caller passes no viewport bounds
    const hasUserBbox = req.neLat !== 0 || req.neLon !== 0 || req.swLat !== 0 || req.swLon !== 0;
    const effectiveReq = hasUserBbox ? req : { ...req, swLat: -75, swLon: -180, neLat: 75, neLon: 180 };
    const requestBounds = normalizeBounds(effectiveReq);
    ...
    const quantizedBB = [
      quantize(effectiveReq.swLat, BBOX_GRID_STEP),
      quantize(effectiveReq.swLon, BBOX_GRID_STEP),
      quantize(effectiveReq.neLat, BBOX_GRID_STEP),
      quantize(effectiveReq.neLon, BBOX_GRID_STEP),
    ].join(':');
    ...
    const fullResult = await cachedFetchJson<ListMilitaryFlightsResponse>(
      ...
      async () => {
        ...
        const fetchBB = {
          lamin: quantize(effectiveReq.swLat, BBOX_GRID_STEP) - BBOX_GRID_STEP / 2,
          lamax: quantize(effectiveReq.neLat, BBOX_GRID_STEP) + BBOX_GRID_STEP / 2,
          lomin: quantize(effectiveReq.swLon, BBOX_GRID_STEP) - BBOX_GRID_STEP / 2,
          lomax: quantize(effectiveReq.neLon, BBOX_GRID_STEP) + BBOX_GRID_STEP / 2,
        };

        let data: { states?: Array<[string, string, ...unknown[]]> } | null = null;

        if (isSidecar || relayBase) {
          const baseUrl = isSidecar ? 'https://opensky-network.org/api/states/all' : relayBase! + '/opensky';
          ...
          try {
            const resp = await fetch(url, { headers: getRelayHeaders(), signal: AbortSignal.timeout(UPSTREAM_TIMEOUT_MS) });
            if (resp.ok) data = await resp.json() as typeof data;
          } catch { /* fall through to direct fetch */ }
        }

        // Direct OpenSky fetch using OAuth2 client credentials (self-hosted fallback)
        if (!data?.states) {
          data = await fetchOpenSkyDirect(fetchBB.lamin, fetchBB.lomin, fetchBB.lamax, fetchBB.lomax);
        }
