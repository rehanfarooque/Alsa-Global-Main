...
async function getGoldIntelligenceLive(): Promise<GetGoldIntelligenceResponse> {
  try {
    const metalSymbols = ['GC=F', 'SI=F', 'PL=F', 'PA=F'];
    const fxSymbols = XAU_FX.map(c => c.symbol);
    const all = await Promise.allSettled([...metalSymbols, ...fxSymbols].map(fetchStooqQuote));
    const get = (i: number) => all[i].status === 'fulfilled' ? all[i].value : null;
    const gold = get(0); const silver = get(1); const platinum = get(2); const palladium = get(3);
    if (!gold?.price) return emptyResponse();
    ...
    for (let i = 0; i < XAU_FX.length; i++) {
      const cfg = XAU_FX[i];
      const fx = get(4 + i);
      if (!fx?.price || fx.price <= 0) continue;
    ...
    return {
      ...
      platinumPrice,
      palladiumPrice,
      goldSilverRatio: goldPrice > 0 && silverPrice > 0 ? goldPrice / silverPrice : undefined,
      goldPlatinumPremiumPct: goldPrice > 0 && platinumPrice > 0
        ? ((goldPrice - platinumPrice) / platinumPrice) * 100 : undefined,
      crossCurrencyPrices,
      drivers: [],
...
export async function getGoldIntelligence(
  ...
  try {
    ...
    if (!rawQuotes || !Array.isArray(rawQuotes) || rawQuotes.length === 0) {
      // Redis unavailable — fall back to live Stooq quotes for core metals.
      return await getGoldIntelligenceLive();
    }
