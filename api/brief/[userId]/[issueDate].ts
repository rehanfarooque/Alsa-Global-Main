...
function renderErrorPage(heading: string, body: string): string {
  return (
    ...
    + `<title>${heading} · AlsaGlobal</title>`
    + `<style>${ERROR_PAGE_STYLES}</style>`
    + '</head><body><div>'
    + `<h1>${heading}</h1>`
    + `<p>${body}</p>`
    + '<p><a href="https://worldmonitor.app">Return to AlsaGlobal</a></p>'
    + '</div></body></html>'
  );
}
...
const FORBIDDEN_PAGE = renderErrorPage(
  ...
  "The brief link you followed is incomplete or has been tampered with. Open the most recent notification from AlsaGlobal to read today's brief.",
);
