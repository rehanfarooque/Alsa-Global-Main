 ...
 *   2. Browser tester key (X-AlsaGlobal-Key) — validated against
...
function hasValidAlsaGlobalKey(key: string): boolean {
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === 'OPTIONS') {
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-AlsaGlobal-Key, X-Api-Key, X-Widget-Key, X-Pro-Key',
      },
    });
  }
  ...
  const worldMonitorKey =
    req.headers.get('X-AlsaGlobal-Key') ??
    req.headers.get('X-Api-Key') ??
    '';
  if (hasValidAlsaGlobalKey(worldMonitorKey)) {
    isPro = true;
  } else {
  ...
  const relayHeaders: Record<string, string> = {
    ...
    'User-Agent': 'alsaglobal-widget-edge/1.0',
