 ...
 *
 *   - env_key → `X-AlsaGlobal-Key: <apiKey>` (existing, unchanged).
...
export async function buildAuthHeaders(
  ...
  if (context.kind === 'env_key') {
    return { 'X-AlsaGlobal-Key': context.apiKey };
  }
  // context.kind === 'pro'
...
export async function resolveAuthContext(
  ...
  const candidateKey = req.headers.get('X-AlsaGlobal-Key') ?? '';
  if (!candidateKey) {
    return {
      ok: false,
      response: new Response(
        JSON.stringify({ jsonrpc: '2.0', id: null, error: { code: -32001, message: 'Authentication required. Use OAuth (/oauth/token) or pass your API key via X-AlsaGlobal-Key header.' } }),
