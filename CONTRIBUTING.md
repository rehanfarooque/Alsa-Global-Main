# Contributing to AlsaGlobal

Thank you for your interest in contributing to AlsaGlobal! This project thrives on community contributions — whether it's code, data sources, documentation, or bug reports.
...
AlsaGlobal is a real-time OSINT dashboard built with **Vanilla TypeScript** (no UI framework), **MapLibre GL + deck.gl** for map rendering, and a custom Proto-first RPC framework called **Sebuf** for all API communication.
...
2. **Clone** your fork locally:
   ```bash
   git clone https://github.com/rehanfarooque/Alsa-Global.git
   cd alsaglobal
   ```
...
- **Internationalization** — help make AlsaGlobal available in more languages
...
- Sebuf handler implementations go in `server/alsaglobal/{domain}/v1/`
...
- Proto definitions go in `proto/alsaglobal/{domain}/v1/`
...
1. **Proto definitions** in `proto/alsaglobal/{domain}/v1/` define services and messages
...
3. **Handlers** in `server/alsaglobal/{domain}/v1/handler.ts` implement the service interface
...
| `docs/api/alsaglobal.openapi.yaml` | **Unified bundle** spanning every service (sebuf ≥ v0.11.0) — use this for external consumers, API explorers, or anywhere you want a single spec covering all RPCs |
...
2. **Add the proto service** (if the data needs a backend proxy) — define messages and RPC methods in `proto/alsaglobal/{domain}/v1/`
3. **Generate stubs** — run `make generate`
4. **Implement the handler** in `server/alsaglobal/{domain}/v1/`
...
rclone copy public/data/country-boundary-overrides.geojson r2:alsaglobal-maps/
```
...
Use the [Bug Report issue template](https://github.com/rehanfarooque/Alsa-Global/issues/new) when available.
...
Use the [Feature Request issue template](https://github.com/rehanfarooque/Alsa-Global/issues/new) when available.
...
Thank you for helping make AlsaGlobal better! 🌍
