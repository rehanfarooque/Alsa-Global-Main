...
test.describe('AI widget builder', () => {
  test.beforeEach(async ({ page }) => {
    await page.addInitScript((key) => {
      if (!sessionStorage.getItem('__widget_e2e_init__')) {
        ...
        localStorage.setItem('alsaglobal-variant', 'happy');
  ...
  test('supports modify, keeps session history, exposes touch-sized controls, and cleans storage on delete', async ({ page }) => {
    ...
    await page.evaluate((widgetId: string) => {
      localStorage.setItem('alsaglobal-panel-spans', JSON.stringify({ [widgetId]: 2 }));
      localStorage.setItem('alsaglobal-panel-col-spans', JSON.stringify({ [widgetId]: 3 }));
    }, storedWidgetMeta!.id);
    ...
    const cleanedStorage = await page.evaluate(() => {
      return {
        ...
        rowSpans: localStorage.getItem('alsaglobal-panel-spans'),
        colSpans: localStorage.getItem('alsaglobal-panel-col-spans'),
      };
    });
