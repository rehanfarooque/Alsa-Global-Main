...
test.describe('desktop runtime routing guardrails', () => {
  ...
  test('cloud fallback blocked without AlsaGlobal API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
  ...
  test('cloud fallback allowed with valid AlsaGlobal API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('worldmonitor.app') && init?.headers) {
          ...
          const wmKey = h.get('X-AlsaGlobal-Key');
          if (wmKey) capturedHeaders['X-AlsaGlobal-Key'] = wmKey;
        }
      ...
      try {
        ...
        return {
          ...
          wmKeyHeader: capturedHeaders['X-AlsaGlobal-Key'] || null,
        };
      } finally {
        window.fetch = originalFetch;
        delete globalWindow.__wmFetchPatched;
