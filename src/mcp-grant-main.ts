 ...
 * a "Sign in with AlsaGlobal Pro" CTA on `api/oauth/authorize.js`).
 *
 * Flow on this page:
...
try {
  const savedTheme = localStorage.getItem('alsaglobal-theme');
...
function errorCodeToMessage(code: string | undefined): string {
  switch (code) {
    ...
    case 'INSUFFICIENT_TIER':
      return 'A AlsaGlobal Pro subscription is required to authorize MCP clients.';
