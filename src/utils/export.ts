...
export function exportToJSON(data: ExportData, filename = 'alsaglobal-export'): void {
  const jsonStr = JSON.stringify(sanitizeData(data), null, 2);
  downloadFile(jsonStr, `${filename}.json`, 'application/json');
}

export function exportToCSV(data: ExportData, filename = 'alsaglobal-export'): void {
  const clean = sanitizeData(data);
  const lines: string[] = [];

  lines.push(`# AlsaGlobal Export — ${new Date(clean.timestamp).toISOString()}`);
