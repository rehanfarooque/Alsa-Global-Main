...
export class DesktopUpdater implements AppModule {
  ...
  private async checkForUpdate(): Promise<void> {
    // AlsaGlobal: desktop auto-updater disabled (no AlsaGlobal release feed).
    // Set DESKTOP_UPDATER_ENABLED = true and point fetch() at your own
    // version endpoint to re-enable.
    const DESKTOP_UPDATER_ENABLED = false;
    if (!DESKTOP_UPDATER_ENABLED) return;
    try {
