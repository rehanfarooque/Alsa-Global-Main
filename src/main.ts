...
Object.defineProperty(window, 'beta', {
  get() {
    const on = localStorage.getItem('alsaglobal-beta-mode') === 'true';
    console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
    return on;
  },
  set(v: boolean) {
    if (v) localStorage.setItem('alsaglobal-beta-mode', 'true');
    else localStorage.removeItem('alsaglobal-beta-mode');
    location.reload();
  },
});
