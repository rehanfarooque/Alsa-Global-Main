...
function renderSecretInput(key: RuntimeSecretKey, _featureId: RuntimeFeatureId): string {
  ...
  if (key === 'GEMINI_MODEL') {
    const GEMINI_MODELS = [
      { value: 'gemini-3.5-flash',          label: 'Gemini 3.5 Flash — latest, fast + high quality ✓' },
      { value: 'gemini-3.1-pro-preview',    label: 'Gemini 3.1 Pro Preview — highest quality' },
      { value: 'gemini-3-pro-preview',      label: 'Gemini 3 Pro Preview — flagship' },
      { value: 'gemini-2.5-pro',            label: 'Gemini 2.5 Pro — stable, 50 req/day free' },
      { value: 'gemini-2.5-flash',          label: 'Gemini 2.5 Flash — stable, 250 req/day free' },
      { value: 'gemini-2.0-flash',          label: 'Gemini 2.0 Flash — ultra-fast' },
    ];
    const storedModel = pending
      ? settingsManager.getPending(key) || ''
      : getRuntimeConfigSnapshot().secrets[key]?.value || 'gemini-3.5-flash';
    const options = GEMINI_MODELS.map(m =>
      `<option value="${m.value}"${storedModel === m.value ? ' selected' : ''}>${escapeHtml(m.label)}</option>`
    ).join('');
    return `
      <div class="settings-secret-row">
        ...
        <select data-static-model-select data-secret="${key}" data-feature="${_featureId}" class="${inputClass}">
          ${options}
        </select>
  ...
  if (key === 'GROQ_MODEL') {
    const GROQ_MODELS = [
      ...
      { value: 'moonshotai/kimi-k2-instruct',    label: 'Kimi K2 — 1T MoE reasoning' },
      { value: 'qwen-qwq-32b',                   label: 'Qwen QwQ 32B — reasoning' },
      { value: 'mistral-saba-24b',               label: 'Mistral Saba 24B' },
    ];
    const storedModel = pending
      ? settingsManager.getPending(key) || ''
      : getRuntimeConfigSnapshot().secrets[key]?.value || 'llama-3.3-70b-versatile';
  ...
  if (key === 'OPENROUTER_MODEL') {
    const OR_MODELS = [
      ...
      { value: 'anthropic/claude-opus-4',            label: 'Claude Opus 4 — Anthropic flagship' },
      ...
      { value: 'openai/gpt-4o',                      label: 'GPT-4o — OpenAI flagship' },
      ...
      { value: 'x-ai/grok-3-beta',                   label: 'Grok 3 Beta — xAI' },
    ];
    const storedModel = pending
      ? settingsManager.getPending(key) || ''
      : getRuntimeConfigSnapshot().secrets[key]?.value || 'google/gemini-2.5-pro';
    const options = OR_MODELS.map(m =>
      `<option value="${m.value}"${storedModel === m.value ? ' selected' : ''}>${escapeHtml(m.label)}</option>`
    ).join('');
    return `
      <div class="settings-secret-row">
...
function initFeatureSectionListeners(area: HTMLElement): void {
  ...
  area.querySelectorAll<HTMLSelectElement>('select[data-static-model-select]').forEach(sel => {
    sel.addEventListener('change', () => {
      const secretKey = sel.dataset.secret as RuntimeSecretKey | undefined;
      const featureId = sel.dataset.feature as RuntimeFeatureId | undefined;
      const model = sel.value;
      if (model && secretKey) {
        settingsManager.setPending(secretKey, model);
        settingsManager.setValidation(secretKey, true);
        sel.classList.remove('invalid');
        sel.classList.add('valid-staged');
        if (featureId) updateFeatureCardStatus(featureId);
        renderSidebar();
      }
    });
  });
