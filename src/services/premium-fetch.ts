 ...
 *      and steps aside (no `X-AlsaGlobal-Key: wms_…` is attached).
 ...
 *   3. api/_api-key.js `validateApiKey()` reads ONLY X-AlsaGlobal-Key.
 ...
 * — those keys travel via X-AlsaGlobal-Key which works on any path.
 */
...
export async function premiumFetch(
  ...
  if (existing.has('Authorization') || existing.has('X-AlsaGlobal-Key')) {
  ...
  try {
    ...
    if (wmKey) {
      existing.set('X-AlsaGlobal-Key', wmKey);
  ...
  for (const testerKey of testerKeys) {
    ...
    testerHeaders.set('X-AlsaGlobal-Key', testerKey);
