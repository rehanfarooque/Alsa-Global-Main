...
// AlsaGlobal: no remote API base. All RPC calls are served by the local Vite
// dev server (sebufApiPlugin) at `/api/*`. Empty string here means "use the
// browser's current origin", which is exactly what we want on localhost.
...
function isAlsaGlobalWebHost(hostname: string): boolean {
...
export function getConfiguredWebApiBaseUrl(): string {
  ...
  if (!isAlsaGlobalWebHost(hostname)) {
    return '';
  }
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const cloudFallback = async () => {
      ...
      if (KEYED_CLOUD_API_PATTERN.test(target)) {
        ...
        if (wmKeyValue) {
          cloudHeaders.set('X-AlsaGlobal-Key', wmKeyValue);
        }
      }
...
export function installWebApiRedirect(): void {
   ...
   *   2. WORLDMONITOR_API_KEY from runtime config → X-AlsaGlobal-Key
  ...
  const enrichInitForPremium = async (pathWithQuery: string, init?: RequestInit): Promise<RequestInit | undefined> => {
    ...
    if (testerKey) {
      headers.set('X-AlsaGlobal-Key', testerKey);
      return { ...withCredentials(init), headers };
    }
