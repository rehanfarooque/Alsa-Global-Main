...
const TOGGLES_STORAGE_KEY = 'alsaglobal-runtime-feature-toggles';
...
export const RUNTIME_FEATURES: RuntimeFeatureDefinition[] = [
  ...
  },
  {
    ...
    description: 'Google Gemini (2.5 Pro / 2.5 Flash) via OpenAI-compatible endpoint.',
    requiredSecrets: ['GEMINI_API_KEY', 'GEMINI_MODEL'],
    fallback: 'Falls back to Groq, then OpenRouter, then local browser model.',
  },
  {
    id: 'aiGroq',
    name: 'Groq summarization',
    description: 'Fast LLM provider — Llama 3.3 70B and other models, free tier with high quota.',
    requiredSecrets: ['GROQ_API_KEY', 'GROQ_MODEL'],
    fallback: 'Falls back to OpenRouter, then local browser model.',
  },
  {
    id: 'aiOpenRouter',
    name: 'OpenRouter summarization',
    description: 'Access 200+ models (Claude, GPT-4o, Gemini Pro, DeepSeek, Grok) via one API key.',
    requiredSecrets: ['OPENROUTER_API_KEY', 'OPENROUTER_MODEL'],
