...
// If a caller already set Authorization / X-AlsaGlobal-Key / X-Api-Key, we
...
export function installWmSessionFetchInterceptor(): void {
  ...
  window.fetch = async function wmSessionFetch(input, init) {
    ...
    // X-AlsaGlobal-Key=wms_... here, the premium injector sees the header
