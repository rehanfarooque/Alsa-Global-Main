...
import type {
  ...
  StrategicProduct,
} from '@/generated/server/alsaglobal/supply_chain/v1/service_server';
...
export class CountryImpactTab {
  ...
  private renderMissing(): void {
    this.element.innerHTML =
      ...
      '<p>AlsaGlobal does not have bilateral trade data for this destination country yet.</p>' +
      '</div>';
  }
  ...
  private renderLazy(): void {
    this.element.innerHTML =
      ...
      '<p>AlsaGlobal is fetching trade data for this destination for the first time. ' +
  ...
  private renderData(data: GetRouteImpactResponse): void {
    const bannerHtml = !data.hs2InSeededUniverse
      ? '<div class="re-impact__banner">Lane value for this HS code is not in AlsaGlobal\'s strategic-products dataset. Top strategic products shown below.</div>'
      : '';
