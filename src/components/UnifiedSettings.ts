    ...
    if (!hasFeature('apiAccess')) {
      ...
      return `
        <div class="panel-locked-state">
          ...
          <div class="panel-locked-desc">Create and manage API keys to access AlsaGlobal data programmatically.</div>
    ...
    return `
      <div class="api-keys-section">
        <div class="api-keys-header">
          <p class="api-keys-desc">Create API keys to access AlsaGlobal data programmatically. Keys are shown once on creation — store them securely.</p>
        </div>
  ...
  private renderMcpClientsContent(): string {
    ...
    if (!hasFeature('mcpAccess')) {
      ...
      return `
        <div class="panel-locked-state">
          <div class="panel-locked-icon">${upgradeIcon}</div>
          <div class="panel-locked-desc">Connect Claude Desktop and other AI clients to your AlsaGlobal account.</div>
        </div>`;
    }
    ...
    return `
      <div class="mcp-clients-section">
        <div class="mcp-clients-header">
          <p class="mcp-clients-desc">Connect Claude Desktop, Cursor, and other AI clients to your AlsaGlobal account. Each client gets its own credential — revoke any time.</p>
        </div>
    ...
    if (active.length === 0 && revoked.length === 0) {
      ...
      container.innerHTML = `
        <div class="mcp-clients-empty">
          ...
          <div class="mcp-clients-empty-desc">To connect Claude Desktop or another AI client, paste this URL into the client's MCP server settings and sign in with your AlsaGlobal Pro account:</div>
