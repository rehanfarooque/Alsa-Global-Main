/**
 * LatestBriefPanel — dashboard surface for the AlsaGlobal Brief.
 *
...
const WM_LOGO_SVG: TrustedHtml = trustedHtml(
  ...
  'Static AlsaGlobal logo SVG defined in source',
);
    ...
    this.content.appendChild(
      h('div', { className: 'latest-brief-card latest-brief-card--composing' },
        ...
        h('div', { className: 'latest-brief-empty-body' },
          'Your personalised brief is tied to your AlsaGlobal account. Sign in to see today\u2019s issue.',
        ),
      ),
    );
  }
  ...
  private renderUpgradeRequired(): void {
    ...
    this.content.appendChild(
      h('div', { className: 'latest-brief-card latest-brief-card--composing' },
        ...
        h('div', { className: 'latest-brief-empty-body' },
          'The AlsaGlobal Brief is included with the Pro plan. Upgrade to unlock today\u2019s issue.',
        ),
      ),
    );
  }
  ...
  private renderComposing(data: LatestBriefComposing): void {
    ...
    this.content.appendChild(
      h('div', { className: 'latest-brief-card latest-brief-card--composing' },
        ...
        h('div', { className: 'latest-brief-empty-body' },
          `The editorial team at AlsaGlobal is writing your ${data.issueDate} brief. Check back in a moment.`,
        ),
      ),
    );
  }
  ...
  private renderReady(data: LatestBriefReady): void {
    ...
    const coverCard = h('a', {
      ...
      h('div', { className: 'latest-brief-cover' },
        ...
        h('div', { className: 'latest-brief-cover-title' }, 'AlsaGlobal'),
    ...
    const shareBtn = h('button', {
      ...
      'aria-label': 'Share AlsaGlobal — copies a referral link',
      disabled: true,
    }, 'Share ↗');
