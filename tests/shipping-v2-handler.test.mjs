...
function proCtx() {
  return makeCtx({ 'X-AlsaGlobal-Key': 'pro-test-key' });
}
...
describe('ShippingV2Service handlers', () => {
  beforeEach(async () => {
    ...
    const riMod = await import('../server/alsaglobal/shipping/v2/route-intelligence.ts');
    const rwMod = await import('../server/alsaglobal/shipping/v2/register-webhook.ts');
    const lwMod = await import('../server/alsaglobal/shipping/v2/list-webhooks.ts');
    const dwMod = await import('../server/alsaglobal/shipping/v2/deliver-webhook.ts');
    webhookShared = await import('../server/alsaglobal/shipping/v2/webhook-shared.ts');
    ...
    const gen = await import('../src/generated/server/alsaglobal/shipping/v2/service_server.ts');
  ...
  describe('registerWebhook', () => {
    ...
    it('rejects callers without an API key with 401 (tenant-isolation gate)', async () => {
      // Without this gate, Clerk-authenticated pro callers with no X-AlsaGlobal-Key
  ...
  describe('deliverWebhook', () => {
    ...
    it('sends only after a public delivery-time DNS answer and includes delivery headers', async () => {
      ...
      assert.equal(seen.headers['user-agent'], 'AlsaGlobal-ShippingV2-Webhooks/1.0');
