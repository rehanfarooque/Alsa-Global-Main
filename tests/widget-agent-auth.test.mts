...
describe('widget-agent unified tester key auth', () => {
  ...
  it('accepts X-AlsaGlobal-Key and upgrades relay request to pro', async () => {
    const res = await handler(new Request('https://www.worldmonitor.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.worldmonitor.app',
        'Content-Type': 'application/json',
        'X-AlsaGlobal-Key': 'browser-test-key',
      },
    ...
    assert.equal(headers.get('X-AlsaGlobal-Key'), null);
  ...
  it('falls back to legacy tester keys when X-AlsaGlobal-Key is invalid', async () => {
    const res = await handler(new Request('https://www.worldmonitor.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.worldmonitor.app',
        'Content-Type': 'application/json',
        'X-AlsaGlobal-Key': 'wrong-key',
  ...
  it('rejects invalid X-AlsaGlobal-Key before relay fetch', async () => {
    const res = await handler(new Request('https://www.worldmonitor.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.worldmonitor.app',
