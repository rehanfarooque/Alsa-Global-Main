/**
 * Tests for server/alsaglobal/aviation/v1/list-airport-delays.ts
 *
...
describe('list-airport-delays: source structure (regression net for #3707)', () => {
  const src = readFileSync(
    resolve(root, 'server/alsaglobal/aviation/v1/list-airport-delays.ts'),
    'utf-8',
  );
...
describe('FlightDelaySeverity proto enum carries an UNKNOWN variant (#3707)', () => {
  it('proto/.../airport_delay.proto declares FLIGHT_DELAY_SEVERITY_UNKNOWN', () => {
    const protoSrc = readFileSync(
      resolve(root, 'proto/alsaglobal/aviation/v1/airport_delay.proto'),
      'utf-8',
    );
  ...
  it('generated server type union includes FLIGHT_DELAY_SEVERITY_UNKNOWN', () => {
    const serverGen = readFileSync(
      resolve(root, 'src/generated/server/alsaglobal/aviation/v1/service_server.ts'),
      'utf-8',
    );
  ...
  it('generated client type union includes FLIGHT_DELAY_SEVERITY_UNKNOWN', () => {
    const clientGen = readFileSync(
      resolve(root, 'src/generated/client/alsaglobal/aviation/v1/service_client.ts'),
      'utf-8',
    );
  ...
  it('_shared.toProtoSeverity maps "unknown" -> FLIGHT_DELAY_SEVERITY_UNKNOWN', () => {
    const sharedSrc = readFileSync(
      resolve(root, 'server/alsaglobal/aviation/v1/_shared.ts'),
      'utf-8',
    );
...
before(async () => {
  ...
  const mod = await import('../server/alsaglobal/aviation/v1/list-airport-delays.ts');
