...
describe('premiumFetch', () => {
  ...
  it('passthrough when Authorization header already set', async () => {
    ...
    assert.equal(sentHeaders().get('X-AlsaGlobal-Key'), null);
  });

  it('passthrough when X-AlsaGlobal-Key header already set', async () => {
    setup();
    await premiumFetch(TARGET, { headers: { 'X-AlsaGlobal-Key': 'caller-key' } });
    assert.equal(fetchMock.mock.calls.length, 1);
    assert.equal(sentHeaders().get('X-AlsaGlobal-Key'), 'caller-key');
  });
  ...
  it('tester key: valid key accepted — exactly one fetch, key forwarded', async () => {
    ...
    assert.equal(sentHeaders().get('X-AlsaGlobal-Key'), 'valid-gateway-key');
  });
  ...
  it('tester key: 401 falls through to Clerk JWT (two fetches)', async () => {
    ...
    assert.equal(sentHeaders(0).get('X-AlsaGlobal-Key'), 'widget-only-key');
    ...
    assert.equal(sentHeaders(1).get('X-AlsaGlobal-Key'), null);
  });
  ...
  it('wm-pro-key 401 retries with wm-widget-key before Clerk', async () => {
    ...
    assert.equal(sentHeaders(0).get('X-AlsaGlobal-Key'), 'relay-only-pro-key');
    assert.equal(sentHeaders(0).get('Authorization'), null);
    assert.equal(sentHeaders(1).get('X-AlsaGlobal-Key'), 'valid-widget-key');
  ...
  it('non-premium path: tester key still attached (works on any path)', async () => {
    ...
    assert.equal(sentHeaders(0).get('X-AlsaGlobal-Key'), 'valid-key');
    assert.equal(sentHeaders(0).get('Authorization'), null);
  });
