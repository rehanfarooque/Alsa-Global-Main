...
function proCtx(path) {
  return makeCtx({ 'X-AlsaGlobal-Key': 'pro-test-key' }, path);
}
...
describe('supply-chain handlers: input-shape validation returns 400, not empty-200', () => {
  beforeEach(async () => {
    ...
    const gcpMod = await import('../server/alsaglobal/supply-chain/v1/get-country-products.ts');
    const gmscMod = await import('../server/alsaglobal/supply-chain/v1/get-multi-sector-cost-shock.ts');
    getCountryProducts = gcpMod.getCountryProducts;
    getMultiSectorCostShock = gmscMod.getMultiSectorCostShock;
    const gen = await import('../src/generated/server/alsaglobal/supply_chain/v1/service_server.ts');
  ...
  describe('getCountryProducts', () => {
    ...
    it('PRO-gate deny on a well-formed iso2 still returns empty-200 (not 400) — intentional contract shift preserved', async () => {
      // No X-AlsaGlobal-Key header → isCallerPremium returns false.
