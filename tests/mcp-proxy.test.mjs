...
function buildHeaders(origin, { authed = true, extra = {} } = {}) {
  ...
  if (authed) h['X-AlsaGlobal-Key'] = ENTERPRISE_KEY;
  return h;
}
...
describe('api/mcp-proxy', () => {
  ...
  describe('Auth gate', () => {
    it('returns 401 when no X-AlsaGlobal-Key is provided', async () => {
    ...
    it('rejects a wms_ session token even though it is technically valid', async () => {
      ...
      const req = new Request(url.toString(), {
        method: 'GET',
        headers: { origin: 'https://worldmonitor.app', 'X-AlsaGlobal-Key': SESSION_TOKEN },
      });
    ...
    it('rejects wm_ user keys when Convex validation cannot run / returns null', async () => {
      ...
      const req = new Request(url.toString(), {
        method: 'GET',
        headers: { origin: 'https://worldmonitor.app', 'X-AlsaGlobal-Key': 'wm_user_abc123' },
      });
      const res = await handler(req);
      assert.equal(res.status, 401);
    });
  ...
  describe('CORS and method handling', () => {
    ...
    it('returns 405 for DELETE', async () => {
      const res = await handler(new Request('https://worldmonitor.app/api/mcp-proxy', {
        method: 'DELETE',
        headers: { origin: 'https://worldmonitor.app', 'X-AlsaGlobal-Key': ENTERPRISE_KEY },
      }));
  ...
  describe('GET /api/mcp-proxy (list tools)', () => {
    ...
    it('ignores invalid JSON in headers param', async () => {
      ...
      const req = new Request(url.toString(), { method: 'GET', headers: { origin: 'https://worldmonitor.app', 'X-AlsaGlobal-Key': ENTERPRISE_KEY } });
      const res = await handler(req);
      assert.equal(res.status, 200);
    });
