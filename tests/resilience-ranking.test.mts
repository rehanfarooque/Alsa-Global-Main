...
import { getResilienceRanking } from '../server/alsaglobal/resilience/v1/get-resilience-ranking.ts';
import {
  ...
  buildRankingItem,
  sortRankingItems,
} from '../server/alsaglobal/resilience/v1/_shared.ts';
...
describe('resilience ranking contracts', () => {
  ...
  it('?refresh=1 is rejected without a valid X-AlsaGlobal-Key (Pro bearer token is NOT enough)', async () => {
    ...
    try {
      ...
      // No X-AlsaGlobal-Key → refresh must be ignored, stale cache returned.
      ...
      const wrongKey = new Request('https://example.com/api/resilience/v1/get-resilience-ranking?refresh=1', {
        headers: { 'X-AlsaGlobal-Key': 'bogus' },
      });
      ...
      const authed = new Request('https://example.com/api/resilience/v1/get-resilience-ranking?refresh=1', {
        headers: { 'X-AlsaGlobal-Key': 'seed-secret' },
      });
