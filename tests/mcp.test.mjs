...
describe('api/mcp.ts — PRO MCP Server', () => {
  ...
  it('returns HTTP 401 + WWW-Authenticate when invalid API key provided', async () => {
    const req = makeReq('POST', initBody(), { 'X-AlsaGlobal-Key': 'wrong_key' });
    const res = await handler(req);
    assert.equal(res.status, 401);
  ...
  it('malformed body returns JSON-RPC -32600', async () => {
    const req = new Request(BASE_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-AlsaGlobal-Key': VALID_KEY },
      body: '{bad json',
    });
    const res = await handler(req);
    const body = await res.json();
...
describe('api/mcp.ts — U7 Pro-path', () => {
  ...
  it('edge: Pro tool _execute fetch sends X-WM-MCP-Internal + X-WM-MCP-User-Id, no X-AlsaGlobal-Key', async () => {
    const { deps } = makeProDeps();
    let captured = null;
    ...
    assert.equal(captured.headers.get('x-alsaglobal-key'), null, 'X-AlsaGlobal-Key must NOT be set for Pro');
