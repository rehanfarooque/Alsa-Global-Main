...
import { adaptSnapshot } from '../server/alsaglobal/intelligence/v1/get-regional-snapshot';

const __dirname = dirname(fileURLToPath(import.meta.url));
const root = resolve(__dirname, '..');

const handlerSrc = readFileSync(
  resolve(root, 'server/alsaglobal/intelligence/v1/get-regional-snapshot.ts'),
  'utf-8',
);

const handlerIndexSrc = readFileSync(
  resolve(root, 'server/alsaglobal/intelligence/v1/handler.ts'),
  'utf-8',
);
...
const protoSrc = readFileSync(
  resolve(root, 'proto/alsaglobal/intelligence/v1/get_regional_snapshot.proto'),
  'utf-8',
);
...
describe('proto definition', () => {
  it('declares the GetRegionalSnapshot RPC method', () => {
    const serviceProtoSrc = readFileSync(
      resolve(root, 'proto/alsaglobal/intelligence/v1/service.proto'),
      'utf-8',
    );
