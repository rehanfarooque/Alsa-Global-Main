...
import { deduplicateHeadlines } from '../server/alsaglobal/news/v1/dedup.mjs';
...
describe('getHumanitarianSummary handler', () => {
  const src = readSrc('server/alsaglobal/conflict/v1/get-humanitarian-summary.ts');
...
describe('humanitarian_summary.proto', () => {
  const proto = readSrc('proto/alsaglobal/conflict/v1/humanitarian_summary.proto');
...
describe('LLM prompt political context (LOW-1)', () => {
  const src = readSrc('server/alsaglobal/news/v1/_shared.ts');
...
describe('getCacheKey determinism', () => {
  ...
  const sharedSrc = readSrc('server/alsaglobal/news/v1/_shared.ts');
...
describe('getVesselSnapshot caching (HIGH-1)', () => {
  const src = readSrc('server/alsaglobal/maritime/v1/get-vessel-snapshot.ts');
...
describe('getSimulationOutcome handler', () => {
  const src = readSrc('server/alsaglobal/forecast/v1/get-simulation-outcome.ts');
