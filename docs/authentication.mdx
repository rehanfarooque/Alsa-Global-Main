...
description: "How user authentication, premium panel gating, and server-side session enforcement work in AlsaGlobal."
---

AlsaGlobal uses [Clerk](https://clerk.com) for authentication. The auth system gates premium panels behind sign-in and tier checks, and enforces session-based access on server-side API endpoints via local JWT verification.
...
1. **Static API key** — `X-AlsaGlobal-Key` header (existing flow, unchanged)
