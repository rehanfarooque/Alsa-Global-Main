...
description: "How AlsaGlobal classifies and publishes fuel shortage alerts (jet, petrol, diesel, heating oil) using an automated evidence-threshold pipeline."
---
...
AlsaGlobal runs every classifier fully automated. Shortage tiers are no exception. Quality comes from **tiered evidence thresholds + an LLM second-pass sanity check + auto-decay of stale rows**, not from a human reviewer.
...
Every public shortage row exposes its `evidenceSources[]` inline — you can read the sources the classifier used. This is what makes AlsaGlobal's shortage registry defensible without an editorial queue.
