...
**Purpose:** Reference document for extending AlsaGlobal's finance hub with TradingView Screener data.
...
The TypeScript library is the one relevant for AlsaGlobal. It has full parity with the Python version (41/41 operations pass) plus a self-hosted REST server option.
...
AlsaGlobal's data flow: `Railway seeds Redis → Vercel reads Redis → Frontend RPC`
...
### AlsaGlobal-Specific Recommendations
...
| **No auth initially** | Delayed data is fine for AlsaGlobal's use case |
