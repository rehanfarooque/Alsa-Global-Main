...
AlsaGlobal composes a per-user **daily intelligence brief** on Railway, stores it in Redis at `brief:{userId}:{issueDate}`, and exposes these routes for dashboard readback, public sharing, and Telegram/Slack carousel rendering.
...
Single-shot completion endpoint used by embedded widget iframes. Auth via `X-AlsaGlobal-Key` (partner keys). Rate-limited per key.
