...
AlsaGlobal has **three** authentication modes. Which one applies depends on how you're calling.
...
| **API key** | `X-AlsaGlobal-Key: wm_live_...` | Server-to-server, scripts, SDKs | All endpoints, including `forceKey: true`. |
| **OAuth bearer** | `Authorization: Bearer <oauth-token>` | MCP clients (Claude, Cursor, Inspector) | `/api/mcp`. The handler also accepts a direct `X-AlsaGlobal-Key` in lieu of an OAuth token — see [MCP](/mcp-server#authentication). |
...
For these, you **must** send `X-AlsaGlobal-Key`.
...
```
X-AlsaGlobal-Key: wm_live_abcdef0123456789...
```
...
2. Else, check `X-AlsaGlobal-Key` against `WORLDMONITOR_VALID_KEYS` (env).
...
- A valid `X-AlsaGlobal-Key` (env-allowlisted from `WORLDMONITOR_VALID_KEYS`, or a user-owned `wm_`-prefixed key whose Convex record has the `apiAccess` entitlement), **or**
