---
title: "API Reference"
description: "Catalog of every AlsaGlobal REST service. All endpoints follow /api/<service>/v1/<rpc-name>."
---

AlsaGlobal exposes its data through a family of versioned REST services. Every endpoint follows the same shape:

```
https://api.worldmonitor.app/api/<service>/v1/<rpc-name>
```

`<rpc-name>` is kebab-case (e.g. `list-acled-events`, `get-resilience-ranking`). Auth is the same on every service — pass `X-AlsaGlobal-Key: wm_live_...` (or rely on a trusted browser origin). See [Authentication](/usage-auth) for details.
