...
![AlsaGlobal Dashboard](images/new-world-monitor.png)

## Platform Variants

AlsaGlobal runs two specialized variants from a single codebase, each optimized for different monitoring needs:

| Variant | URL | Focus |
|---------|-----|-------|
| **🌍 AlsaGlobal** | [worldmonitor.app](https://worldmonitor.app) | Geopolitical intelligence, military tracking, conflict monitoring, infrastructure security |
...
The Service Status panel tracks the operational health of external services that AlsaGlobal users may depend on.
...
// App.ts
const header = `AlsaGlobal v${__APP_VERSION__}`;
```
...
- ✅ **Variant Switcher UI** - Compact orbital navigation between AlsaGlobal and Tech Monitor
