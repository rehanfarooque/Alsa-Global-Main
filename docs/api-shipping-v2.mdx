...
The v2 shipping API is a **PRO-gated** read + webhook-subscription surface on top of AlsaGlobal's chokepoint registry and AIS tracking data.

<Info>
All v2 shipping endpoints require `X-AlsaGlobal-Key` (server-to-server). Browser origins are **not** trusted here — `validateApiKey` runs with `forceKey: true`.
</Info>
...
Auth: `X-AlsaGlobal-Key` (forceKey: true) + PRO. Returns `401` / `403` otherwise.
