...
- **Pro** — sign in with your AlsaGlobal account, no key to manage. 50 tool calls per UTC day.
...
Restart Claude Desktop. The first time you mention AlsaGlobal in a chat, Claude pops the OAuth consent screen — click **Sign in with AlsaGlobal Pro**, authenticate in your browser, and the token is stored locally by Claude. No API key ever touches your filesystem.
...
Claude will pick `get_market_data` from the AlsaGlobal toolset, call it with no arguments, and answer in plain English. The raw tool response is a single bootstrap bundle covering quotes, sector ETFs, crypto, gulf quotes, ETF flows, and the AlsaGlobal fear-greed composite. Typical latency: **300–800 ms** (cache read from Redis, no upstream API call).
...
The `wm_live_…` key goes in `X-AlsaGlobal-Key`, **not** as a `Bearer` token — sending it as a bearer fails OAuth resolution and returns `401 invalid_token`. If you already have an OAuth access token from `/api/oauth/token`, use `Authorization: Bearer $TOKEN` instead and drop the `X-AlsaGlobal-Key` header.
