...
- `api/_api-key.js` validates `X-AlsaGlobal-Key` header on sebuf routes
...
`X-AlsaGlobal-Key` is allowed in both `server/cors.ts` and `api/_cors.js`.
...
| `server/cors.ts` / `api/_cors.js` | CORS headers with `X-AlsaGlobal-Key` |
| `src/components/AlsaGlobalTab.ts` | Settings UI for key + registration |
