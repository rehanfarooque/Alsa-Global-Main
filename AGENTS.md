...
Agent entry point for AlsaGlobal. Read this first, then follow links for depth.
...
│   └── alsaglobal/       # Domain handlers (mirrors proto service structure)
├── proto/                  # Protobuf definitions (sebuf framework)
│   ├── buf.yaml            # Buf configuration
│   └── alsaglobal/       # Service definitions with HTTP annotations
...
- `server/alsaglobal/<domain>/` has RPC handlers matching proto services
...
1. Define proto message in `proto/alsaglobal/<domain>/`
2. Add RPC with `(sebuf.http.config)` annotation
3. Run `make generate`
4. Create handler in `server/alsaglobal/<domain>/`
