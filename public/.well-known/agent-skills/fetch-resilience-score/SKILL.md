...
`/api/resilience/v1/get-resilience-score` is Pro-tier. Agents and other server-to-server callers MUST present an API key in the `X-AlsaGlobal-Key` header. `Authorization: Bearer …` is for MCP/OAuth or Clerk JWTs — **not** raw API keys.

```
X-AlsaGlobal-Key: wm_live_...
```

The key must be attached to a Pro subscription. Unauthenticated or free-tier requests return `401` / `403`. Issue a key at /
