...
Use this skill when the user asks for a summary of the current geopolitical, economic, or security situation in a specific country. The endpoint returns a fresh AI-generated brief composed from the latest news, market, conflict, and infrastructure signals AlsaGlobal tracks for that country.

## Authentication

Server-to-server callers (agents, scripts, SDKs) MUST present an API key in the `X-AlsaGlobal-Key` header. `Authorization: Bearer …` is for MCP/OAuth or Clerk JWTs — **not** raw API keys.

```
X-AlsaGlobal-Key: wm_live_...
```

Browser requests from `worldmonitor.app` get a free pass via CORS Origin trust, but agents will never hit that path. Issue a key at /
...
```bash
curl -s -H "X-AlsaGlobal-Key: $WM_API_KEY" \
...
```bash
curl -s --get -H "X-AlsaGlobal-Key: $WM_API_KEY" \
...
- `401` — missing `X-AlsaGlobal-Key` (server-to-server callers).
