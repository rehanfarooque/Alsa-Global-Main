  ...
  try {
    const resp = await fetch(BRIEF_WHY_MATTERS_ENDPOINT_URL, {
      ...
      headers: {
        ...
        'User-Agent': 'alsaglobal-digest-notifications/1.0',
...
async function upstashRest(...args) {
  const res = await fetch(`${UPSTASH_URL}/${args.map(encodeURIComponent).join('/')}`, {
    ...
    headers: {
      ...
      'User-Agent': 'alsaglobal-digest/1.0',
    },
...
function formatDigest(stories, nowMs) {
  ...
  const lines = [`AlsaGlobal Daily Digest — ${dateStr}`, ''];
            ...
            <table cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td style="width: 36px; height: 36px; vertical-align: middle;">
                  <img src="https://www.worldmonitor.app/favico/android-chrome-192x192.png" width="36" height="36" alt="AlsaGlobal" style="border-radius: 50%; display: block;" />
                </td>
...
async function sendTelegramBriefCarousel(userId, chatId, caption, magazineUrl) {
  ...
  try {
    const res = await fetch(
      ...
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'User-Agent': 'alsaglobal-digest/1.0' },
...
async function isUserPro(userId) {
  ...
  try {
    const res = await fetch(`${CONVEX_SITE_URL}/relay/entitlement`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${RELAY_SECRET}`, 'User-Agent': 'alsaglobal-digest/1.0' },
...
function buildChannelBodies(storyListPlain, aiSummary, magazineUrl) {
  ...
  const briefFooterPlain = magazineUrl
    ? `\n\n${DIVIDER}\n\n📖 Open your AlsaGlobal Brief magazine:\n${magazineUrl}`
    : '';
  const briefFooterTelegram = magazineUrl
    ? `\n\n${DIVIDER}\n\n📖 <a href="${telegramSafeUrl}">Open your AlsaGlobal Brief magazine</a>`
    : '';
  const briefFooterSlack = magazineUrl
    ? `\n\n${DIVIDER}\n\n📖 <${slackSafeUrl}|Open your AlsaGlobal Brief magazine>`
    : '';
  const briefFooterDiscord = magazineUrl
    ? `\n\n${DIVIDER}\n\n📖 [Open your AlsaGlobal Brief magazine](${magazineUrl})`
    : '';
  if (!aiSummary) {
    return {
...
<a href="${escapedUrl}" style="display:inline-block;background:#f2ede4;color:#0a0a0a;text-decoration:none;font-weight:700;font-size:14px;letter-spacing:0.08em;padding:14px 22px;border-radius:4px;">Open your AlsaGlobal Brief →</a>
