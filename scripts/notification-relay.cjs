...
const RESEND_FROM = process.env.RESEND_FROM_EMAIL ?? 'AlsaGlobal <alerts@worldmonitor.app>';
...
async function upstashRest(...args) {
  const res = await fetch(`${UPSTASH_URL}/${args.map(encodeURIComponent).join('/')}`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${UPSTASH_TOKEN}`, 'User-Agent': 'alsaglobal-relay/1.0' },
  });
  if (!res.ok) {
  ...
  try {
    const res = await fetch(`${CONVEX_SITE_URL}/relay/deactivate`, {
      ...
      headers: {
        ...
        'User-Agent': 'alsaglobal-relay/1.0',
      },
...
async function isUserPro(userId) {
  ...
  try {
    const res = await fetch(`${CONVEX_SITE_URL}/relay/entitlement`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${RELAY_SECRET}`, 'User-Agent': 'alsaglobal-relay/1.0' },
...
async function drainHeldForUser(userId, variant, allowedChannelTypes) {
  ...
  const lines = [`AlsaGlobal — ${events.length} held alert${events.length !== 1 ? 's' : ''} from quiet hours`, ''];
  ...
  const subject = `AlsaGlobal — ${events.length} held alert${events.length !== 1 ? 's' : ''}`;
    ...
    try {
      ...
      if (ch.channelType === 'telegram' && ch.chatId) ok = await sendTelegram(userId, ch.chatId, text);
        ...
        ok = await sendWebPush(userId, ch, {
          title: `AlsaGlobal · ${events.length} held alert${events.length === 1 ? '' : 's'}`,
          body: subject,
...
async function sendTelegram(userId, chatId, text, _retryCount = 0) {
  ...
  const res = await fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'User-Agent': 'alsaglobal-relay/1.0' },
...
async function sendWebPush(userId, subscription, payload) {
  ...
  const body = JSON.stringify({
    title: payload.title || 'AlsaGlobal',
    body: payload.body || '',
    url: payload.url || 'https://worldmonitor.app/',
    tag: payload.tag || 'alsaglobal-generic',
...
async function processWelcome(event) {
  ...
  const text = `✅ AlsaGlobal connected! You'll receive breaking news alerts here.`;
  if (channelType === 'slack' && ch.webhookEnvelope) {
    ...
    await sendEmail(ch.email, 'AlsaGlobal Notifications Connected', text);
...
async function processEvent(event) {
  ...
  const subject = `AlsaGlobal Alert: ${event.payload?.title ?? event.eventType}`;
      ...
      try {
        if (ch.channelType === 'telegram' && ch.chatId) {
          ...
          await sendWebPush(rule.userId, ch, {
            title: event.payload?.title || event.eventType || 'AlsaGlobal',
            body: firstLine,
            url: eventUrl,
