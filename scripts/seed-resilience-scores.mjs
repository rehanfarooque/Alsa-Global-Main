...
const WM_KEY = process.env.WORLDMONITOR_API_KEY
  ...
  || '';
const SEED_UA = 'Mozilla/5.0 (compatible; AlsaGlobal-Seed/1.0)';

// Bumped v13 → v14 in lockstep with server/alsaglobal/resilience/v1/
...
// lockstep with server/alsaglobal/resilience/v1/_dimension-scorers.ts
...
async function seedResilienceScores() {
  ...
  if (missing > 0) {
    ...
    try {
      ...
      if (WM_KEY) headers['X-AlsaGlobal-Key'] = WM_KEY;
      ...
      for (let i = 0; i < stillMissing.length; i += BATCH) {
        ...
        const results = await Promise.allSettled(batch.map(async (cc) => {
          ...
          const resp = await fetch(scoreUrl, {
            headers: { 'User-Agent': SEED_UA, 'Accept': 'application/json', 'X-AlsaGlobal-Key': WM_KEY },
            signal: AbortSignal.timeout(30_000),
          });
...
async function refreshRankingAggregate({ url, token, laggardsWarmed }) {
  ...
  try {
    ...
    if (WM_KEY) rebuildHeaders['X-AlsaGlobal-Key'] = WM_KEY;
