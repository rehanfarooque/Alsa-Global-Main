...
// Source of truth: server/alsaglobal/resilience/v1/_shared.ts → RESILIENCE_SCORE_CACHE_PREFIX
const RESILIENCE_SCORE_CACHE_PREFIX = 'resilience:score:v18:';

// Mirror of server/alsaglobal/resilience/v1/_shared.ts#currentCacheFormula.
...
async function fetchAlsaGlobalScores(url, token, countryCodes) {
...
async function run() {
  ...
  console.log(`Fetching AlsaGlobal resilience scores for ${SAMPLE_COUNTRIES.length} countries...`);
  const wmScores = await fetchAlsaGlobalScores(url, token, SAMPLE_COUNTRIES);
  ...
  console.log(`AlsaGlobal vs ND-GAIN Readiness:  rho = ${ndgainResult.rho.toFixed(3)} (n=${ndgainResult.n}, target > 0.65) ${ndgainPass ? 'PASS' : 'FAIL'}`);
  console.log(`AlsaGlobal vs INFORM Risk:        rho = ${informResult.rho.toFixed(3)} (n=${informResult.n}, target > 0.60, inverted) ${informPass ? 'PASS' : 'FAIL'}`);
  ...
  console.log('AlsaGlobal score ranking (sample):');
